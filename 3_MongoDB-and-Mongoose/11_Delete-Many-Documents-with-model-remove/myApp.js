...
Person.find({name: "Kris" , age:45}, (error, data)=> {
    if(error){
        console.log(error)
      }
      else
...
//find by array will look and retrieve Persons who contains all argument inside them array
Person.find({favoriteFoods : {$all : ["prawns", "lobster"]}}, (error, data) => {
    if(error){
        console.log(error)
      }
      else
...
var removeManyPeople = function(done) {
  ...
  //Person.remove({name: nameToRemove}, (error, deletedRecords) => {
  Person.deleteMany({name: nameToRemove}, (error, deletedRecords) => {
    if(error)
    {
      console.log(error)
    }
